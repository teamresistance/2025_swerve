...
public class ChoreoWrap {
	public static List<String> getAllAutoNames() {
		...
		return Stream.of(autoFiles).filter(file -> !file.isDirectory()).map(File::getName).filter(name -> name.endsWith(".traj")).map(name -> name.substring(0, name.lastIndexOf("."))).collect(Collectors.toList());
	}
	...
	public static void addChoreoAutos(LoggedDashboardChooser<Command> autos, Drive drive) {
		...
		for (String name : autoNames) {
			...
			Command swerveCommand = Choreo.choreoSwerveCommand(traj,
				drive::getPose,
				...
				thetaController,
				(ChassisSpeeds speeds) -> drive.runVelocity(
			...
			autos.addOption("Choreo: " + name, Commands.sequence(
				Commands.runOnce(() -> drive.resetOdometry(traj.getInitialPose())),
				swerveCommand,
				drive.run(() -> drive.runVelocity(
